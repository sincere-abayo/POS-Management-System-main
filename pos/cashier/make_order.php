                ...
                <div class="form-row">
                  <div class="col-md-12 mb-2">
                    <div class="form-check">
                      ...
                      <label class="form-check-label" for="walkinToggle">Unregistered/Walk-in Customer</label>
                    </div>
                  </div>
                  ...
                  <div class="col-md-4 d-none" id="walkinCustomerBlock">
                    ...
                    <input type="text" name="customer_phone" class="form-control" placeholder="Enter phone number"
                      pattern="\d{10}" maxlength="10" minlength="10" inputmode="numeric" id="walkinPhone">
                    <input type="hidden" name="customer_id" value="walkin">
                  </div>
                  <div class="col-md-4">
                    <label>Customer ID</label>
                    <input type="text" name="customer_id" readonly id="customerID" class="form-control" value="walkin">
                  </div>
                  <div class="col-md-4">
                ...
                while ($prod = $res->fetch_object()) {
                  ...
                  <div class="form-row align-items-end mb-3">
                    ...
                    <div class="col-md-4">
                      ...
                      <input type="text" id="totalPrice" class="form-control" value="RWF <?php echo $prod->prod_price; ?>"
                        readonly>
                    </div>
                    <div class="col-md-4 d-flex align-items-end">
                      <input type="submit" name="make" value="Make Order" class="btn btn-success w-100" <?php if ($prod->quantity == 0)
                        echo 'disabled'; ?>>
                    </div>
                  </div>
                  ...
                  <div class="card mb-4">
                    <div class="row no-gutters">
                      <div class="col-md-4 text-center p-3">
                        <img
                          src="../admin/assets/img/products/<?php echo $prod->prod_img ? $prod->prod_img : 'default.jpg'; ?>"
                          class="img-fluid rounded" style="max-height:180px;">
                      </div>
                      <div class="col-md-8">
                        <div class="card-body">
                          <h4 class="card-title mb-1"><?php echo htmlspecialchars($prod->prod_name); ?></h4>
                          <p class="mb-1"><b>Code:</b> <?php echo htmlspecialchars($prod->prod_code); ?> |
                            <b>Category:</b>
                          ...
                          <p class="mb-1"><b>Available Stock:</b> <?php echo $prod->quantity; ?>
                            <?php if ($prod->quantity <= $prod->min_stocks) { ?><span class="badge badge-warning ml-2">Low
                                Stock!</span><?php } ?>
                          </p>
                          <p class="mb-1"><b>Unit Price:</b> RWF<span
                              id="unitPrice"><?php echo $prod->prod_price; ?></span>
                          </p>
                          <p class="mb-2"><b>Description:</b> <?php echo nl2br(htmlspecialchars($prod->prod_desc)); ?></p>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      if (qtyInput && !qtyInput.disabled) {
                        qtyInput.addEventListener('input', function () {
                          ...
                          if (qty > maxQty) {
                            ...
                            qty = maxQty;
                            qtyWarning.textContent = 'Cannot order more than available stock (' + maxQty + ').';
      ...
      if (customerSearch && custNameSelect) {
        customerSearch.addEventListener('input', function () {
          ...
          for (var i = 0; i < custNameSelect.options.length; i++) {
            ...
            var phone = option.getAttribute('data-phone') ? option.getAttribute('data-phone').toLowerCase() : '';
            var email = option.getAttribute('data-email') ? option.getAttribute('data-email').toLowerCase() : '';
            option.style.display = (text.includes(filter) || phone.includes(filter) || email.includes(filter)) ? '' : 'none';
          }
        });
        custNameSelect.addEventListener('change', function () {
          ...
          if ($('input[name="customer_name"]').length && !$('#walkinCustomerBlock').is(':visible')) {
