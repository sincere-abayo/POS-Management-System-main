...
if (isset($_POST['ChangeProfile'])) {
    ...
    if (empty($_POST["customer_phoneno"]) || empty($_POST["customer_name"]) || empty($_POST['customer_email'])) {
        ...
        // Fetch current password so we don't overwrite it
        $getPwdQuery = "SELECT customer_password FROM rpos_customers WHERE customer_id = ?";
        $getPwdStmt = $mysqli->prepare($getPwdQuery);
        $getPwdStmt->bind_param('s', $customer_id);
        $getPwdStmt->execute();
        $getPwdStmt->bind_result($customer_password);
        $getPwdStmt->fetch();
        $getPwdStmt->close();
        //Update profile
                                                    ...
                                                    <div class="pl-lg-4">
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <div class="form-group">
                                                                    <label class="form-control-label"
                                                                        for="input-username">Old Password</label>
                                                            ...
                                                            <div class="col-lg-12">
                                                                <div class="form-group">
                                                                    <label class="form-control-label"
                                                                        for="input-email">Confirm New Password</label>
