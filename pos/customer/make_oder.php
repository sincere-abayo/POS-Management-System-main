...
if (isset($_POST['make'])) {
    ...
    if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price'])) {
        ...
        if ($postStmt) {
            // Display success message with payment code
            $success = "Order Submitted. Please note your payment code: $order_id You will need it to complete the payment.";
            header("refresh:300; url=payments.php"); // Redirect after 50 seconds
        } else {
